...
public class FileManager {
    ...
    public static ArrayList<Object> getSellerData(Seller seller, Store store) {
        ...
        try {
            ...
            while (line != null) {
                String[] arr = line.split(";");
                products.add(arr[0]);
                products.add(new Product(arr[1], arr[2],
                        Double.parseDouble(arr[3]), Integer.parseInt(arr[4])));
                line = br.readLine();
            }
        ...
        for (File f : sellerFolder.listFiles()) {
            ...
            try {
                ...
                while (line != null) {
                    String[] arr = line.split(";");
                    temp.add(arr[0]);
                    temp.add(new Product(arr[1], arr[2],
                            Double.parseDouble(arr[3]), Integer.parseInt(arr[4])));
                    line = br.readLine();
                }
            } catch (Exception e) {
    ...
    public static void addCustomerData(Customer customer,
                                       Product product, int purchaseQuantity) {
        createNecessaryFolders(customer);
        try {
            File histFile = new File(customerDataFolder + File.separatorChar
                    + customer.getUsername() + File.separatorChar + "purchase_history");
            FileWriter fw = new FileWriter(histFile, true);
            fw.write(purchaseQuantity + ";" +
     ...
     * @param purchaseQuantity
     */
    public static void addCustomerShopppingCart(Customer customer,
                                                Product product, int purchaseQuantity) {
        createNecessaryFolders(customer);
        try {
            File histFile = new File(customerDataFolder + File.separatorChar
                    + customer.getUsername() + File.separatorChar + "shopping_cart");
            FileWriter fw = new FileWriter(histFile, true);
            fw.write(purchaseQuantity + ";" + product.getName() + ";" +
     ...
     * @param purchaseQuantity
     */
    public static void updateCustomerShoppingCart(Customer customer,
                                                  Product product, int purchaseQuantity) {
        createNecessaryFolders(customer);
     ...
     * @param soldQuantity
     */
    public static void updateSellerData(Seller seller, Store store,
                                        Product product, int productQuantity, int soldQuantity) {
        createNecessaryFolders(seller);
     ...
     * @param quantity
     */
    public static void updateSellerDataQuantity(Seller seller,
                                                Store store, Product product, int quantity) {
        createNecessaryFolders(seller);
        ArrayList<ArrayList<Object>> currentData = getSellerAllData(seller);
        ArrayList<ArrayList<Object>> tempData = new ArrayList<>(currentData);
        File histFile = new File(sellerDataFolder + File.separatorChar
     ...
     * @param description
     */
    public static void updateSellerDataDescription(Seller seller,
                                                   Store store, Product product, String description) {
        createNecessaryFolders(seller);
        ArrayList<ArrayList<Object>> currentData = getSellerAllData(seller);
        ArrayList<ArrayList<Object>> tempData = new ArrayList<>(currentData);
        File histFile = new File(sellerDataFolder + File.separatorChar
     ...
     * @param price
     */
    public static void updateSellerDataPrice(Seller seller,
                                             Store store, Product product, double price) {
        createNecessaryFolders(seller);
        ArrayList<ArrayList<Object>> currentData = getSellerAllData(seller);
        ArrayList<ArrayList<Object>> tempData = new ArrayList<>(currentData);
        File histFile = new File(sellerDataFolder + File.separatorChar
    ...
    public static void removeSellerDataProduct(Seller seller,
     ...
     * @param file
     */
    public static void replaceSellerData(ArrayList<ArrayList<Object>> data,
                                         File file) {
        try {
            FileWriter fw = new FileWriter(file, false);
